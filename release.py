"""
release.py — Build, tag, and publish a new PriorityShift release.
...
What it does:
    ...
    3. Builds the app via PyInstaller
       - macOS  → PriorityShift.app + PriorityShift.dmg
       - Windows → PriorityShift.exe folder + PriorityShift.zip
       - Linux   → PriorityShift folder + PriorityShift.tar.gz
    4. Updates the version in PriorityShift.spec
...
Note: this builds the desktop Control Panel (main.py), which runs the Flask
app in-process — the packaged app is fully standalone and does not need a
separate Python install or virtualenv to run.
"""
...
APP_NAME  = "PriorityShift"
SPEC_FILE = "PriorityShift.spec"

ROOT = Path(__file__).parent

# Per-platform output artifact
if sys.platform == "darwin":
    ARTIFACT_NAME = "PriorityShift.dmg"
elif sys.platform == "win32":
    ARTIFACT_NAME = "PriorityShift.zip"
else:
    ARTIFACT_NAME = "PriorityShift.tar.gz"
...
def commit_and_tag(version: str) -> str:
    ...
    # Deliberately avoid -A so stray .env / instance / build artefacts aren't
    # swept in.
    run(["git", "add", "--update"])  # tracked modifications
    for path in ("app/", "migrations/", "tests/", "main.py", "seed.py", "wsgi.py", "requirements.txt"):
        run(["git", "add", path, "--"], check=False)
    run(["git", "add", SPEC_FILE, "--"], check=False)
...
def main():
    ...
    print()
    print("╔══════════════════════════════════════════════╗")
    print(f"║     PriorityShift — Release Publisher ({platform_label:<6}) ║")
    print("╚══════════════════════════════════════════════╝")
    print()
